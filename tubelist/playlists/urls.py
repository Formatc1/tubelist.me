...
urlpatterns = patterns('',
    ...
    url(r'^(?P<playlist_id>\w+)/add/(?P<video_id>[a-zA-z0-9_-]+)/(?P<video_name>.+)/$', views.add, name='add'),
    url(r'^(?P<playlist_id>\w+)/delete/(?P<video_id>[a-zA-z0-9_-]+)/$', views.delete, name='delete'),
    )
